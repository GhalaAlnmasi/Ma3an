...
def dashboard(request):
    context = {
        'pending_agencies': Agency.objects.filter(approved=False, rejected=False).count(),
        'active_agencies': Agency.objects.filter(approved=True).count(),
        'total_users': Traveler.objects.count(),
...
def manage_agencies(request):
    ...
    agencies_qs = Agency.objects.select_related('profile', 'profile__user').all()
...
def agency_detail(request, agency_id):
    agency = get_object_or_404(Agency.objects.select_related('profile', 'profile__user'), id=agency_id)
...
def agency_detail(request, agency_id):
    agency = get_object_or_404(
        Agency.objects.select_related('profile', 'profile__user'),
        id=agency_id
    )
    return render(request, 'backOffice/agency_detail.html', {'agency': agency})
